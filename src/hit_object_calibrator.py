...
class HitObjectCalibrator:
    ...
    def run(self):
        """Starts the interactive hit object calibration tool."""
        ...
        while True:
            ...
            if key == 27:
                ...
                self.hit_circle_params = {'param1': p1, 'param2': p2, 'minRadius': min_r, 'maxRadius': max_r}
                self.calibration_data['hit_circle_params'] = self.hit_circle_params
                self._save_calibration_data()
                print("Hit circle parameters saved.")
                break
            elif key == ord('m'):
                current_frame_pos = min(self.total_frames - 1, current_frame_pos + 2)
                self.cap.set(cv2.CAP_PROP_POS_FRAMES, current_frame_pos)
                ret, frame = self.cap.read()
            elif key == ord('n'):
                current_frame_pos = max(0, current_frame_pos - 2)
                self.cap.set(cv2.CAP_PROP_POS_FRAMES, current_frame_pos)
                ret, frame = self.cap.read()
